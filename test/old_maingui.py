...
def main():
    ...
    # visualization_speed = (
    #     st.sidebar.slider("Vitesse de la visualisation", 0.01, 1.0, 0.1, 0.01)
    #     if show_visualization
    #     else 0
    # )
    # visualization_episodes = (
    #     st.sidebar.slider("Nombre d'épisodes à visualiser", 1, 10, 3, step=1)
    #     if show_visualization
    #     else 0
    # )
    ...
    def train_and_test():
        ...
        with st.spinner("Entraînement des modèles en cours..."):
            ...
            dqn_train_rewards, dqn_train_steps = solver.dqn.train_dqn(training_episodes)
        ...
        with st.spinner("Test des modèles en cours..."):
            ...
            q_avg_reward = round(np.mean(q_test_rewards), 2)
            dqn_avg_reward = round(np.mean(dqn_test_rewards), 2)
            q_avg_steps = round(np.mean(q_test_steps), 2)
            dqn_avg_steps = round(np.mean(dqn_test_steps), 2)
    ...
    if mode == "Mode utilisateur":
        ...
        st.sidebar.slider("Temps limite (secondes)", 1, 300, 60, key="time_limit")
    ...
    if st.session_state.get("can_save_dqn", False):
        ...
        if st.button("Sauvegarder le modèle DQN", key="save_dqn_btn"):
            print(f"Chemin de sauvegarde: {path_to_save}")
            # solver.dqn.save_model(path_to_save)
